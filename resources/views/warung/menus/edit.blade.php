...
<div class="max-w-2xl mx-auto">
    <div class="bg-white/70 backdrop-blur-xl border border-white/80 rounded-3xl shadow-sm p-6 md:p-8">
        ...
        <form method="POST" action="{{ route('warung.menus.update', $menu) }}" enctype="multipart/form-data" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full rounded-xl border border-zinc-200 bg-white/60 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400/30 focus:border-amber-400 transition resize-none">{{ old('description', $menu->description) }}</textarea>
            </div>
            <div>
                ...
                <div id="previewWrap" class="{{ $menu->photo ? '' : 'hidden' }} mb-3">
                    <img id="photoPreview" src="{{ $menu->photo ? asset('storage/'.$menu->photo) : '' }}" alt="{{ $menu->name }}" class="h-40 w-full object-cover rounded-2xl border border-zinc-200">
                    @if($menu->photo)<p class="text-xs text-zinc-400 mt-1.5">Foto saat ini — upload baru untuk mengganti</p>@endif
                </div>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit" class="flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium px-5 py-2.5 rounded-xl transition">
                    <iconify-icon icon="solar:check-circle-linear"></iconify-icon>Update Menu
                </button>
