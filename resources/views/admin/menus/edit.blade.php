    ...
    <div class="bg-white/70 backdrop-blur-xl border border-white/80 rounded-3xl shadow-sm p-6 md:p-8">
        ...
        <div class="flex items-center gap-3 mb-6">
            <div class="w-10 h-10 rounded-2xl bg-zinc-900 flex items-center justify-center">
                <iconify-icon icon="solar:pen-linear" class="text-white text-lg"></iconify-icon>
            </div>
            <div>
                <h2 class="text-lg font-semibold text-zinc-900 leading-tight">Edit Menu</h2>
                <p class="text-sm text-zinc-500">{{ $menu->name }}</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.menus.update', $menu) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="category" value="{{ old('category', $menu->category) }}" required list="categories"
                        class="w-full rounded-xl border border-zinc-200 bg-white/60 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900/20 focus:border-zinc-400 transition">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="status" required
                        ...
                        <option value="out"       {{ old('status', $menu->status) == 'out'       ? 'selected' : '' }}>Habis</option>
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3"
                    class="w-full rounded-xl border border-zinc-200 bg-white/60 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-zinc-900/20 focus:border-zinc-400 transition resize-none">{{ old('description', $menu->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                {{-- Current / preview --}}
                <div id="previewWrap" class="{{ $menu->photo ? '' : 'hidden' }} mb-3">
                    <img id="photoPreview"
                         src="{{ $menu->photo ? asset('storage/'.$menu->photo) : '' }}"
                         alt="{{ $menu->name }}"
                         class="h-40 w-full object-cover rounded-2xl border border-zinc-200">
                    @if($menu->photo)
                    <p class="text-xs text-zinc-400 mt-1.5">Foto saat ini — upload baru untuk mengganti</p>
                    @endif
                </div>

                <label class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-zinc-300 rounded-2xl cursor-pointer bg-zinc-50/60 hover:bg-zinc-100/60 transition group">
                    <iconify-icon icon="solar:upload-linear" class="text-2xl text-zinc-400 group-hover:text-zinc-600 mb-1 transition"></iconify-icon>
                    <span class="text-sm text-zinc-500 group-hover:text-zinc-700 transition">Klik untuk ganti foto</span>
            ...
            <div class="flex gap-3 pt-2">
                <button type="submit"
                    class="flex items-center gap-2 bg-zinc-900 hover:bg-zinc-700 text-white text-sm font-medium px-5 py-2.5 rounded-xl transition">
                    <iconify-icon icon="solar:check-circle-linear"></iconify-icon>
                    Update Menu
                </button>
                <a href="{{ route('admin.menus.index') }}"
                    class="flex items-center gap-2 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 text-sm font-medium px-5 py-2.5 rounded-xl transition">
